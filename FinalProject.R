...
# Teresa - Helped with Recursive Partitioning and debugging KNN
...
knnElec = function(k, var, winner){
  ...
  knn2004 = data.frame(county = na.omit(FinalDF)$county,
                       state =  na.omit(FinalDF)$state,
                       Total.Pop = na.omit(FinalDF)$Total.Pop,
                       knnpredict2004 = k2004, weighofk = attr(k2004,"prob"))
  return(knn2004)
}
...
###plot 1(Part 4)####
plot(c(sum(as.character(knn2004$knnpredict2004)=="Dem"),
         ...
         c(sum(
          ...
          (knn2004[knn2004$knnpredict2004=="Rep","Total.Pop"]*
            (1-knn2004[knn2004$knnpredict2004=="Rep","weighofk"])))/(10^7),
                  sum(
                    knn2004[knn2004$knnpredict2004=="Rep","Total.Pop"]*
                    knn2004[knn2004$knnpredict2004=="Rep","weighofk"],
                    (knn2004[knn2004$knnpredict2004=="Dem","Total.Pop"]*
                    (1-knn2004[knn2004$knnpredict2004=="Dem","weighofk"])))/(10^7)
                  ),
        col= c("#010440", "#A61103"),
        xlab = c("Counties"),
        ylab = c("Number of votes in millions"),
        sub = c("A comparision of the number of counties won by Democrats and Republicans in 2012 based on variables
                regarding citizenship and  attainment of at least an undergraduate degree, 
...
makeKNN2012plot = function(file) {
  pdf(file=file, width = 9.9, height = 7.7)
  barplot(c(sum(as.character(knn2004$knnpredict2004)=="Dem"),
            sum(as.character(knn2004$knnpredict2004)=="Rep")),
          width = c(sum(FinalDF$obama.votes, na.rm=TRUE), 
                    sum(FinalDF$romney.votes, na.rm=TRUE)),
          col= c("#010440", "#A61103"),
          xlab = c("Counties"),
          ylab = c("Number of votes"),
          sub = c("the actual result from 2012."))
  dev.off()
}

makeKNN2012plot("knn2012plot.pdf")

###Plot 2(Part 4)####
Predicting04outcome = function(){
  knnVars = c("ForeignBorn", "BuisSciArt.Occup",
              "AgroForestFishMine.Occup", "Total.Pop")
  knnpredict2004 = knnElec(k=3, var=knnVars, "winner2004")
  return(knnpredict2004)
}
knn04= Predicting04outcome()

#makeKNNplot = function(file) {
#  pdf(file=file, width = 9.9, height = 7.7)
  barplot(c(sum(as.character(knn04$winner2004)=="Dem"),
            sum(as.character(knn04$winner2004)=="Rep")),
          width = c(sum(
          ...
          xlab = c("Counties"),
          ylab = c("Number of votes"),
          sub = c("Comparision unemployment and Foreign Born factors, height indicating county and width indicating 
                  population.
                  While Republican still have an advantage over county,
                  width shows population isn't a big landslide"), cex.sub= 0.5)
#  dev.off()
#}

#makeKNNplot("knnBarPlot.pdf")
...
FinalDF$Winner2012 = winners(FinalDF$romney.votes, FinalDF$obama.votes)
...
### PLOTS 1 and 2 ###
### Accuracy of RPART and KNN predictors ###
...
incorrectColor = function(party) {
  ### Takes a character vector with party name "Rep" or "Dem".
  ### Returns green if Dem, and beige if Rep. Uses Brewer Hex Codes.
...
createPredMap = function(predictions, winners, lon, lat, file, type, desc) {
  ...
  points(lon/1000000, lat/1000000, col=color2, pch=20)
  dev.off()  
}

## Create pred map for recursive partitioning
createPredMap(FinalDF$Predictions2004, FinalDF$winner2012, FinalDF$longitude,
              FinalDF$latitude, "rpartPredMap.pdf",
...
knnVars = c("Attain.BachAndUp", "Total.Pop")
knnDF = na.omit(FinalDF[, c("state", "county", "bushVote", "kerryVote", "winner2012", 
                            "longitude", "latitude", knnVars)])
createPredMap(knn2004$knnpredict2004, knnDF$winner2012, knnDF$longitude,
              knnDF$latitude, "knnPredMap.pdf",
              "K-Nearest Neighbor",
...
makeArrowMap = function(file) {
  ...
  pdf(file=file, width = 9.9, height = 7.65) 
  map("county", fill=TRUE, col="#BDBDBD44", border="white")
